...
def get_gene_suggestions_(db_file, text):
    '''
    Get a list of autocomplete suggestions for genes containing
    the text.
    ...
    query = """
        SELECT importance, fields, startPos, endPos FROM intervals
        WHERE name LIKE '%{}%' and fields LIKE '%gene'
        ORDER BY importance DESC
        LIMIT 10
        """.format(text)
    ...
    to_return = []
    for (importance, fields, startPos, endPos) in rows:
        field_parts = fields.split('\t')
        to_return += [{
                      'chr': field_parts[0],
                      'txStart': startPos,
                      'txEnd': endPos,
                      'txStart_': int(field_parts[1]),
                      'txEnd_': int(field_parts[2]),
                      'score': importance,
                      'geneName': field_parts[3]}]
